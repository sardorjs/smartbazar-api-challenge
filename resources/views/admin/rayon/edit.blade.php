<x-app-layout>
    <x-slot name="header">
        {{ __('Edit rayon') }} - {{ $rayon->name }}
    </x-slot>

    <div class="content bg-white shadow p-4 rounded">
        <form action="{{ route('admin.rayon.update', $rayon) }}" method="POST" enctype="multipart/form-data"
                ...
                @if ($cities->isNotEmpty())
                    ...
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pr-6 pl-2.5 py-2.5">
                        ...
                        @foreach ($cities as $city)
                            <option value="{{ $city->id }}" {{ $rayon->city_id === $city->id ? 'selected' : '' }}>{{ $city->name }}</option>
                        @endforeach
                    </select>
                @else
            ...
            <div>
                ...
                <input type="text" id="name" name="name"  placeholder="e.g: Mirabad district" required value="{{ $rayon->name }}"
